...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    StackParams get_stack_params() {
      ...
      output.start_with_holding = params.start_with_holding;
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        ...
        if (param.get_name() == (prefix_ + "start_with_holding")) {
            updated_params.start_with_holding = param.as_bool();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "start_with_holding")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "(Deprecated) If true, start with holding position after activation. Otherwise, no command will be sent until the first trajectory is received.";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.start_with_holding);
          parameters_interface_->declare_parameter(prefix_ + "start_with_holding", parameter, descriptor);
      }
      ...
      updated_params.interpolation_method = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "start_with_holding");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.start_with_holding = param.as_bool();
