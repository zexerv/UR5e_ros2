...
#include "rosidl_runtime_c/string.h"  // class_type, name, plugin_name, type
#include "rosidl_runtime_c/string_functions.h"  // class_type, name, plugin_name, type
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = controller_manager_msgs__msg__HardwareComponentState;
    is_plain =
      (
...
static size_t _HardwareComponentState__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_controller_manager_msgs__msg__HardwareComponentState(
