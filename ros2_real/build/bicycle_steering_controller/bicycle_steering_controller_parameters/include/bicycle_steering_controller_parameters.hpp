...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    void declare_params(){
      ...
      if(auto validation_result = gt<double>(param, 0.0);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'front_wheel_radius': {}", validation_result.error()));
      }
      updated_params.front_wheel_radius = param.as_double();
      ...
      if(auto validation_result = gt<double>(param, 0.0);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'rear_wheel_radius': {}", validation_result.error()));
      }
      updated_params.rear_wheel_radius = param.as_double();
